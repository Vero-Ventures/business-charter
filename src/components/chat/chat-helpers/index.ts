...
export const validateChatSettings = (
    ...
    return await processResponse(
        response,
        isRegeneration ? payload.chatMessages[payload.chatMessages.length - 1] : tempAssistantMessage,
        false,
        newAbortController,
    ...
    if (provider === "google") {
        formattedMessages = await buildGoogleGeminiFinalMessages(payload, profile, newMessageImages);
    } else {
        formattedMessages = await buildFinalMessages(payload, profile, chatImages);
    }

    const apiEndpoint = provider === "custom" ? "/api/chat/custom" : `/api/chat/${provider}`;
    ...
    if (isRegeneration) {
        ...
        const createdMessages = await createMessages([finalUserMessage, finalAssistantMessage]);
